...
<template>
  ...
  <Sidebar v-model:visible="visible" position="right" class="layout-config-sidebar w-20rem">
    ...
    <div class="flex align-items-center">
      <Button icon="pi pi-minus" type="button" class="p-button-text p-button-rounded w-2rem h-2rem mr-2"
        :disabled="layoutConfig.scale.value === scales[0]" @click="decrementScale()" />
      <div class="flex gap-2 align-items-center">
        <i v-for="scale in scales" :key="scale" class="pi pi-circle-fill text-300"
          :class="{ 'text-primary-500': scale === layoutConfig.scale.value }" />
      </div>
      <Button icon="pi pi-plus" type="button" class="p-button-text p-button-rounded w-2rem h-2rem ml-2"
        :disabled="layoutConfig.scale.value === scales[scales.length - 1]" @click="incrementScale()" />
    </div>
    ...
    <template v-if="!simple">
      ...
      <div class="flex">
        <div class="field-radiobutton flex-1">
          <RadioButton v-model="layoutConfig.inputStyle.value" name="inputStyle" value="outlined"
